...
from mmedit.datasets import DATASETS, RepeatDataset, build_dataloader, build_dataset
...
def test_build_dataset():
    ...
    cfg = dict(type="ToyDatasetWithAnnFile", ann_file=["ann_file_a", "ann_file_b"])
    dataset = build_dataset(cfg)
    ...
    # test concat dataset
    cfg = (dict(type="ToyDataset"), dict(type="ToyDatasetWithAnnFile", ann_file="ann_file"))
    dataset = build_dataset(cfg)
    assert isinstance(dataset, ConcatDataset)
    assert isinstance(dataset.datasets, list)
