...
def test_visual_hook():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        visual_hook = VisualizationHook(tmpdir, ["img"], interval=8, rerange=False)
        runner = mmcv.runner.IterBasedRunner(model=model, optimizer=None, work_dir=tmpdir, logger=get_root_logger())
        runner.register_hook(visual_hook)
        runner.run([data_loader], [("train", 10)], 10)
        img_saved = mmcv.imread(osp.join(tmpdir, "iter_8.png"), flag="unchanged")

        np.testing.assert_almost_equal(img_saved, img[0].permute(1, 2, 0) * 255)

    with tempfile.TemporaryDirectory() as tmpdir:
        visual_hook = VisualizationHook(tmpdir, ["img"], interval=8, rerange=True)
        runner = mmcv.runner.IterBasedRunner(model=model, optimizer=None, work_dir=tmpdir, logger=get_root_logger())
        runner.register_hook(visual_hook)
        runner.run([data_loader], [("train", 10)], 10)
        img_saved = mmcv.imread(osp.join(tmpdir, "iter_8.png"), flag="unchanged")
