...
class TestEMA:
    ...
    def setup_class(cls):
        cls.default_config = dict(
            module_keys=("module_a_ema", "module_b_ema"), interval=1, interp_cfg=dict(momentum=0.5)
        )
    ...
    def test_ema_hook(self):
        ...
        assert torch.equal(ema_states["a"], torch.tensor([1.0, 2.0]))
        ...
        ema.after_train_iter(self.runner)
        ema_states = module_a_ema.state_dict()
        assert torch.equal(self.runner.model.module_a.a, torch.tensor([0.5, 1.0]))
        assert torch.equal(ema_states["a"], torch.tensor([0.75, 1.5]))
        assert torch.equal(ema_states["b"], torch.tensor([1.0, 1.5]))
        ...
        with pytest.raises(AssertionError):
            _ = ExponentialMovingAverageHook(module_keys=("module_a_ema"), interp_mode="xxx")
        ...
        ema_states = module_a_ema.state_dict()
        assert torch.equal(runner.model.module_a.a, torch.tensor([0.5, 1.0]))
        assert torch.equal(ema_states["a"], torch.tensor([0.5, 1.0]))
    ...
    def test_ema_hook_cuda(self):
        ...
        ema_states = module_a_ema.state_dict()
        assert torch.equal(ema_states["a"], torch.tensor([1.0, 2.0]).cuda())
        ...
        ema.after_train_iter(cuda_runner)
        ema_states = module_a_ema.state_dict()
        assert torch.equal(cuda_runner.model.module_a.a, torch.tensor([0.5, 1.0]).cuda())
        assert torch.equal(ema_states["a"], torch.tensor([0.75, 1.5]).cuda())
        assert torch.equal(ema_states["b"], torch.tensor([1.0, 1.5]).cuda())
        assert "c" not in ema_states
        ...
        ema.after_train_iter(self.runner)
        ema_states = module_a_ema.state_dict()
        assert torch.equal(self.runner.model.module.module_a.a, torch.tensor([0.5, 1.0]).cuda())
        assert torch.equal(ema_states["a"], torch.tensor([0.75, 1.5]).cuda())
        assert torch.equal(ema_states["b"], torch.tensor([1.0, 1.5]).cuda())
        assert "c" not in ema_states
        ...
        ema.after_train_iter(runner)
        ema_states = module_a_ema.state_dict()
        assert torch.equal(runner.model.module_a.a, torch.tensor([0.5, 1.0]).cuda())
        assert torch.equal(ema_states["a"], torch.tensor([0.5, 1.0]).cuda())

        module_a.a.data /= 2
        runner.iter += 2
        ema.after_train_iter(runner)
        ema_states = module_a_ema.state_dict()
        assert torch.equal(runner.model.module_a.a, torch.tensor([0.25, 0.5]).cuda())
