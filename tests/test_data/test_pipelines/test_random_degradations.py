...
def test_random_noise():
    ...
    # Gaussian noise
    model = RandomNoise(
        params=dict(noise_type=["gaussian"], noise_prob=[1], gaussian_sigma=[0, 50], gaussian_gray_noise_prob=1),
        keys=["lq"],
    )
    ...
    # Poisson noise
    model = RandomNoise(
        params=dict(noise_type=["poisson"], noise_prob=[1], poisson_scale=[0, 1], poisson_gray_noise_prob=1),
        keys=["lq"],
    )
    results = model(results)
    assert results["lq"].shape == (8, 8, 3)
...
def test_random_video_compression():
    ...
    model = RandomVideoCompression(
        params=dict(codec=["libx264", "h264", "mpeg4"], codec_prob=[1 / 3.0, 1 / 3.0, 1 / 3.0], bitrate=[1e4, 1e5]),
        keys=["lq"],
    )
    results = model(results)
    ...
    # skip degradations with prob < 1
    params = dict(
        codec=["libx264", "h264", "mpeg4"], codec_prob=[1 / 3.0, 1 / 3.0, 1 / 3.0], bitrate=[1e4, 1e5], prob=0
    )
...
def test_degradations_with_shuffle():
    ...
    assert (
        repr(model)
        == model.__class__.__name__ + f"(degradations={degradations}, " + "keys=['lq'], " + "shuffle_idx=(1, 2))"
    )
