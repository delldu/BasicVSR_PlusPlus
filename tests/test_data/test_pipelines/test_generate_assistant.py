...
from mmedit.datasets.pipelines import GenerateCoordinateAndCell, GenerateHeatmap
...
def test_generate_coordinate_and_cell():
    ...
    coordinate2 = GenerateCoordinateAndCell(sample_quantity=64 * 48, scale=3.1, target_size=(128, 96))
    results2 = coordinate2(inputs2)
    assert set(list(results2.keys())) == set(["gt", "coord", "cell"])
    assert results2["gt"].shape == (64 * 48, 3)

    inputs3 = dict()
    coordinate3 = GenerateCoordinateAndCell(sample_quantity=64 * 48, scale=3.1, target_size=(128, 96))
