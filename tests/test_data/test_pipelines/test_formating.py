...
from mmedit.datasets.pipelines import Collect, FormatTrimap, GetMaskedImage, ImageToTensor, ToTensor
...
def test_to_tensor():
    ...
    ori_results = dict(
        tensor=torch.randn(2, 3), numpy=np.random.randn(2, 3), sequence=list(range(10)), int=1, float=0.1
    )
    ...
    ori_results = dict(
        tensor=torch.randn(2, 3), numpy=np.random.randn(2, 3), sequence=list(range(10)), int=1, float=0.1, str="test"
    )
    results = to_tensor(ori_results)
    assert check_keys_contain(results.keys(), target_keys)
    for key in target_keys:
