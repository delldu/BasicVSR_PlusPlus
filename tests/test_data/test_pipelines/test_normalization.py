...
class TestAugmentations:
    ...
    def test_normalize(self):
        with pytest.raises(TypeError):
            Normalize(["alpha"], dict(mean=[123.675, 116.28, 103.53]), [58.395, 57.12, 57.375])

        with pytest.raises(TypeError):
            Normalize(["alpha"], [123.675, 116.28, 103.53], dict(std=[58.395, 57.12, 57.375]))

        target_keys = ["merged", "img_norm_cfg"]

        merged = np.random.rand(240, 320, 3).astype(np.float32)
        results = dict(merged=merged)
        config = dict(keys=["merged"], mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], to_rgb=False)
        normalize = Normalize(**config)
        normalize_results = normalize(results)
        assert self.check_keys_contain(normalize_results.keys(), target_keys)
        self.check_normalize(merged, normalize_results["merged"], normalize_results["img_norm_cfg"])

        merged = np.random.rand(240, 320, 3).astype(np.float32)
        results = dict(merged=merged)
        config = dict(keys=["merged"], mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], to_rgb=True)
        normalize = Normalize(**config)
        normalize_results = normalize(results)
        assert self.check_keys_contain(normalize_results.keys(), target_keys)
        self.check_normalize(merged, normalize_results["merged"], normalize_results["img_norm_cfg"])
        ...
        assert normalize.__repr__() == (
            normalize.__class__.__name__ + f"(keys={ ['merged']}, mean={np.array([123.675, 116.28, 103.53])},"
        ...
        normalize_results = normalize(results)
        assert self.check_keys_contain(normalize_results.keys(), target_keys)
        self.check_normalize(merged, normalize_results["merged"][0], normalize_results["img_norm_cfg"])
        self.check_normalize(merged_2, normalize_results["merged"][1], normalize_results["img_norm_cfg"])
    ...
    def test_rescale_to_zero_one(self):
        ...
        assert self.check_keys_contain(rescale_to_zero_one_results.keys(), target_keys)
        assert rescale_to_zero_one_results["alpha"].shape == (240, 320)
        np.testing.assert_almost_equal(rescale_to_zero_one_results["alpha"], alpha / 255.0)
        assert repr(rescale_to_zero_one) == (rescale_to_zero_one.__class__.__name__ + f"(keys={['alpha']})")
        ...
        np.testing.assert_almost_equal(rescale_to_zero_one_results["alpha"][0], alpha / 255.0)
        np.testing.assert_almost_equal(rescale_to_zero_one_results["alpha"][1], alpha_2 / 255.0)
