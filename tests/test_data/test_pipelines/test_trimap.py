...
def generate_ref_trimap(alpha, kernel_size, iterations, random):
    ...
    src = alpha
    pad = ((dilate_kh // 2, (dilate_kh - 1) // 2), (dilate_kw // 2, (dilate_kw - 1) // 2))
    for _ in range(dilate_iter):
        src = np.pad(src, pad, constant_values=np.min(src))
        for i in range(h):
            for j in range(w):
                target = src[i : i + dilate_kh, j : j + dilate_kw]
                dilated[i, j] = np.max((target * dilate_kernel)[dilate_kernel == 1])
        src = dilated
...
def test_generate_trimap():
    ...
    assert repr(generate_trimap) == (
        generate_trimap.__class__.__name__ + f"(kernels={kernels}, iterations={(iterations, iterations + 1)}, "
        f"random=True)"
    )
...
def test_generate_trimap_with_dist_transform():
    ...
    assert repr(generate_trimap) == (generate_trimap.__class__.__name__ + "(dist_thr=3, random=True)")
...
def test_composite_fg():
    ...
    results = dict(alpha=alpha, fg=fg, bg=bg)
    composite_fg = CompositeFg(
        f"tests{os.sep}data{os.sep}fg", f"tests{os.sep}data{os.sep}alpha", interpolation="bilinear"
    )
    composite_fg_results = composite_fg(results)
    assert check_keys_contain(composite_fg_results.keys(), target_keys)
    assert composite_fg_results["fg"].shape == (32, 32, 3)
    ...
    assert repr(composite_fg) == composite_fg.__class__.__name__ + strs.replace("\\", "\\\\")
...
def test_generate_soft_seg():
    ...
    with pytest.raises(TypeError):
        ...
        GenerateSoftSeg(border_width=25.0)
    with pytest.raises(TypeError):
        # erode_ksize must be an int
        GenerateSoftSeg(erode_ksize=5.0)
    with pytest.raises(TypeError):
        # dilate_ksize must be an int
        GenerateSoftSeg(dilate_ksize=5.0)
    with pytest.raises(TypeError):
    ...
    generate_soft_seg = GenerateSoftSeg(
        erode_ksize=3, dilate_ksize=3, erode_iter_range=(1, 2), dilate_iter_range=(1, 2), blur_ksizes=[(11, 11)]
    )
...
def test_transform_trimap():
    ...
    assert results_transformed["transformed_trimap"].shape[:2] == dummy_trimap.shape
