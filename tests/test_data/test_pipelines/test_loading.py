...
def test_load_image_from_file():
    ...
    path_baboon_x4 = Path(__file__).parent.parent.parent / "data" / "lq" / "baboon_x4.png"
    ...
    assert repr(image_loader) == (
        ...
        + ("(io_backend=disk, key=lq, " "flag=color, save_original_img=False, channel_order=bgr, " "use_cache=False)")
    )
    ...
    assert repr(image_loader) == (
        image_loader.__class__.__name__
        + ("(io_backend=disk, key=gt, " "flag=color, save_original_img=False, channel_order=bgr, " "use_cache=True)")
    )
    results = image_loader(results)
...
def test_load_image_from_file_list():
    ...
    # convert to y-channel (rgb2y)
    results = dict(lq_path=[str(path_baboon_x4), str(path_baboon)])
    config = dict(io_backend="disk", key="lq", channel_order="rgb", convert_to="y")
    image_loader = LoadImageFromFileList(**config)
    results = image_loader(results)
    np.testing.assert_almost_equal(results["lq"][0], img_baboon_x4_y)
    np.testing.assert_almost_equal(results["lq"][1], img_baboon_y)
...
class TestMattingLoading:
    ...
    def test_random_load_bg(self):
        ...
        for _ in range(2):
            ...
            assert self.check_keys_contain(random_load_bg_results.keys(), target_keys)
            assert isinstance(random_load_bg_results["bg"], np.ndarray)
            assert random_load_bg_results["bg"].shape == (128, 128, 3)

        assert repr(random_load_bg) == random_load_bg.__class__.__name__ + ("(bg_dir='tests/data/bg')")


class TestInpaintLoading:
    @classmethod
    def setup_class(cls):
        cls.img_path = Path(__file__).parent.parent.parent.joinpath("data/image/test.png")
    ...
    def test_load_mask(self):
        ...
        for _ in range(2):
            ...
            gt_mask = mmcv.imread("./tests/data/mask/test.png", flag="unchanged")
            assert np.array_equal(results["mask"], gt_mask[..., 0:1] / 255.0)
        ...
        results = dict()
        results = set_loader(results)
        gt_mask = mmcv.imread("./tests/data/mask/test_single_ch.png", flag="unchanged")
        gt_mask = np.expand_dims(gt_mask, axis=2)
        assert np.array_equal(results["mask"], gt_mask[..., 0:1] / 255.0)

        # test mask mode: ff
        mask_config = dict(
            img_shape=(256, 256), num_vertices=(4, 12), mean_angle=1.2, angle_range=0.4, brush_width=(12, 40)
        )
        ...
        mask = mask_loader(dict(mask_path="./tests/data/mask/test_single_ch.png"))
...
class TestGenerationLoading:
    ...
    def test_load_paired_image_from_file(self):
        ...
        config = dict(io_backend="disk", key="pair", flag="grayscale", save_original_img=True)
        results = copy.deepcopy(self.results)
        load_paired_image_from_file = LoadPairedImageFromFile(**config)
        results = load_paired_image_from_file(results)
...
def test_dct_mask():
    ...
    for i in range(mask_width):
        for j in range(mask_height):
            mask_values[i, j] = max(
                gamma ** (min(i, mask_width - i - 1) * beta), gamma ** (min(j, mask_height - j - 1) * beta)
            )
    ...
    repr_str = dct_mask.__class__.__name__ + (f"(gamma={dct_mask.gamma}, " f"beta={dct_mask.beta})")
