...
from mmedit.datasets.pipelines import (
    Crop,
    CropAroundCenter,
    CropAroundFg,
    CropAroundUnknown,
...
class TestAugmentations:
    ...
    def setup_class(cls):
        ...
        cls.results = dict(lq=cls.img_lq, gt=cls.img_gt, scale=4, lq_path="fake_lq_path", gt_path="fake_gt_path")
    ...
    def test_crop(self):
        ...
        assert str(random_crop) == (
            random_crop.__class__.__name__ + "keys=['img'], crop_size=(512, 512), random_crop=True"
        )

        # test center crop for size larger than original shape
        results = copy.deepcopy(self.results)
        center_crop = Crop(["img"], crop_size=(512, 512), random_crop=False, is_pad_zeros=True)
        gt_pad = np.pad(self.results["img"], ((128, 128), (128, 128), (0, 0)), mode="constant", constant_values=0)
        results = center_crop(results)
    ...
    def test_random_resized_crop(self):
        ...
        with pytest.raises(TypeError):
            RandomResizedCrop(["img"], crop_size=(128, 128), scale=(0.5, 0.5), ratio=(1, 2))
        ...
        assert str(random_resized_crop) == (
            random_resized_crop.__class__.__name__ + "(keys=['img'], crop_size=(128, 128), scale=(0.08, 1.0), "
        ...
        assert str(random_resized_crop) == (
            random_resized_crop.__class__.__name__ + "(keys=['img'], crop_size=(512, 512), scale=(0.08, 1.0), "
            f"ratio={(3. / 4., 4. / 3.)}, interpolation=bilinear)"
        )

        # test center crop for in_ratio < min(self.ratio)
        results = copy.deepcopy(self.results)
        center_crop = RandomResizedCrop(["img"], crop_size=(128, 128), ratio=(100.0, 200.0))
        results = center_crop(results)
        ...
        results = copy.deepcopy(self.results)
        center_crop = RandomResizedCrop(["img"], crop_size=(128, 128), ratio=(0.01, 0.02))
        results = center_crop(results)
    ...
    def test_fixed_crop(self):
        ...
        # test given pos crop
        results = copy.deepcopy(self.results)
        given_pos_crop = FixedCrop(["img_a", "img_b"], crop_size=(256, 256), crop_pos=(1, 1))
        ...
        results = copy.deepcopy(self.results)
        given_pos_crop = FixedCrop(["img_a", "img_b"], crop_size=(256, 256), crop_pos=(280, 280))
        results = given_pos_crop(results)
        ...
        results = copy.deepcopy(self.results)
        random_fixed_crop = FixedCrop(["img_a", "img_b"], crop_size=(256, 256), crop_pos=None)
        ...
        assert np.array_equal(
            self.results["img_a"][y_offset : y_offset + crop_h, x_offset : x_offset + crop_w, :], results["img_a"]
        )
        assert np.array_equal(
            self.results["img_b"][y_offset : y_offset + crop_h, x_offset : x_offset + crop_w, :], results["img_b"]
        )

        # test given pos crop for lager size than the original shape
        results = copy.deepcopy(self.results)
        given_pos_crop = FixedCrop(["img_a", "img_b"], crop_size=(512, 512), crop_pos=(1, 1))
        results = given_pos_crop(results)
        ...
        assert str(given_pos_crop) == (
            given_pos_crop.__class__.__name__ + "keys=['img_a', 'img_b'], crop_size=(512, 512), crop_pos=(1, 1)"
        )

        # test random initialized fixed crop for lager size
        # than the original shape
        results = copy.deepcopy(self.results)
        random_fixed_crop = FixedCrop(["img_a", "img_b"], crop_size=(512, 512), crop_pos=None)
        results = random_fixed_crop(results)
        ...
        assert str(random_fixed_crop) == (
            random_fixed_crop.__class__.__name__ + "keys=['img_a', 'img_b'], crop_size=(512, 512), crop_pos=None"
        )

    def test_crop_around_center(self):

        with pytest.raises(TypeError):
            CropAroundCenter(320.0)
        ...
        assert self.check_keys_contain(crop_around_center_results.keys(), target_keys)
        assert self.check_crop(crop_around_center_results["alpha"].shape, crop_around_center_results["crop_bbox"])
        ...
        repr_str = crop_around_center.__class__.__name__ + (f"(crop_size={(200, 200)})")
    ...
    def test_crop_around_unknown(self):
        ...
        target_keys = ["fg", "bg", "merged", "alpha", "trimap", "ori_merged", "crop_bbox"]
        ...
        results = dict(fg=fg, bg=bg, merged=merged, ori_merged=ori_merged, alpha=alpha, trimap=trimap)
        crop_around_semi_trans = CropAroundUnknown(keys, crop_sizes=[320], unknown_source="trimap")
        crop_around_semi_trans_results = crop_around_semi_trans(results)
        assert self.check_keys_contain(crop_around_semi_trans_results.keys(), target_keys)
        assert self.check_crop(
            crop_around_semi_trans_results["alpha"].shape, crop_around_semi_trans_results["crop_bbox"]
        )
        assert self.check_crop_around_semi(crop_around_semi_trans_results["alpha"])

        keys = ["fg", "bg", "merged", "alpha", "ori_merged"]
        target_keys = ["fg", "bg", "merged", "alpha", "ori_merged", "crop_bbox"]
        ...
        results = dict(fg=fg, bg=bg, merged=merged, ori_merged=ori_merged, alpha=alpha)
        crop_around_semi_trans = CropAroundUnknown(keys, crop_sizes=[160], unknown_source="alpha")
        crop_around_semi_trans_results = crop_around_semi_trans(results)
        assert self.check_keys_contain(crop_around_semi_trans_results.keys(), target_keys)
        assert self.check_crop(
            crop_around_semi_trans_results["alpha"].shape, crop_around_semi_trans_results["crop_bbox"]
        ...
        alpha = np.zeros((240, 320))
        results = dict(fg=fg, bg=bg, merged=merged, ori_merged=ori_merged, alpha=alpha)
        crop_around_semi_trans = CropAroundUnknown(keys, crop_sizes=[240], unknown_source="alpha")
        crop_around_semi_trans_results = crop_around_semi_trans(results)
        assert self.check_keys_contain(crop_around_semi_trans_results.keys(), target_keys)
        assert self.check_crop(
            crop_around_semi_trans_results["alpha"].shape, crop_around_semi_trans_results["crop_bbox"]
    ...
    def test_crop_around_fg(self):
        ...
        assert self.check_keys_contain(crop_around_fg_results.keys(), target_keys)
        assert self.check_crop(crop_around_fg_results["seg"].shape, crop_around_fg_results["crop_bbox"])

        crop_around_fg = CropAroundFg(keys, test_mode=True)
        crop_around_fg_results = crop_around_fg(results)
        result_img_shape = crop_around_fg_results["seg"].shape
        assert self.check_keys_contain(crop_around_fg_results.keys(), target_keys)
        assert self.check_crop(result_img_shape, crop_around_fg_results["crop_bbox"])
    ...
    def test_paired_random_crop(self):
        ...
        assert repr(pairedrandomcrop) == (pairedrandomcrop.__class__.__name__ + "(gt_patch_size=128)")
