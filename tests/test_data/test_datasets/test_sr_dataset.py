...
class TestSRDatasets:
    ...
    def test_sr_folder_dataset(self):
        ...
        sr_folder_dataset = SRFolderDataset(
            lq_folder=lq_folder, gt_folder=gt_folder, pipeline=sr_pipeline, scale=4, filename_tmpl=filename_tmpl
        )
    ...
    def test_sr_folder_gt_dataset(self):
        ...
        sr_folder_dataset = SRFolderGTDataset(
            gt_folder=gt_folder, pipeline=sr_pipeline, scale=4, filename_tmpl=filename_tmpl
        )
        data_infos = sr_folder_dataset.data_infos
    ...
    def test_sr_landmark_dataset(self):
        ...
        sr_pipeline = [
            dict(
                type="LoadImageFromFile", io_backend="disk", key="gt", flag="color", channel_order="rgb", backend="cv2"
            )
        ]
        ...
        sr_landmark_dataset = SRFacialLandmarkDataset(
            gt_folder=gt_folder, ann_file=ann_file, pipeline=sr_pipeline, scale=4
        )
        ...
        # input path is str
        sr_landmark_dataset = SRFacialLandmarkDataset(
            gt_folder=str(gt_folder), ann_file=str(ann_file), pipeline=sr_pipeline, scale=4
        )
        data_infos = sr_landmark_dataset.data_infos
        assert len(data_infos) == 1
        result = sr_landmark_dataset[0]
    ...
    def test_sr_lmdb_dataset(self):
        ...
        sr_pipeline = [
            dict(type="LoadImageFromFile", io_backend="lmdb", key="lq", db_path=lq_lmdb_folder),
            dict(type="LoadImageFromFile", io_backend="lmdb", key="gt", db_path=lq_lmdb_folder),
            dict(type="ImageToTensor", keys=["lq", "gt"]),
        ]
        target_keys = ["lq_path", "gt_path", "scale", "lq", "lq_ori_shape", "gt", "gt_ori_shape"]
        ...
        sr_lmdb_dataset = SRLmdbDataset(
            lq_folder=lq_lmdb_folder, gt_folder=lq_lmdb_folder, pipeline=sr_pipeline, scale=1  # fake gt_folder
        )
        ...
        # input path is str
        sr_lmdb_dataset = SRLmdbDataset(
            lq_folder=str(lq_lmdb_folder), gt_folder=(lq_lmdb_folder), pipeline=sr_pipeline, scale=1  # fake gt_folder
        )
        data_infos = sr_lmdb_dataset.data_infos
        assert data_infos == [dict(lq_path="baboon", gt_path="baboon")]
        result = sr_lmdb_dataset[0]
        ...
        with pytest.raises(ValueError):
            sr_lmdb_dataset = SRLmdbDataset(
                lq_folder=lq_lmdb_folder, gt_folder=self.data_prefix, pipeline=sr_pipeline, scale=1  # normal folder
            )
        with pytest.raises(ValueError):
            sr_lmdb_dataset = SRLmdbDataset(
...
def test_vimeo90k_dataset():
    ...
    with patch("builtins.open", mocked_open_function):
        ...
        assert vimeo90k_dataset.data_infos == [
            dict(lq_path=lq_paths_1, gt_path=gt_paths_1, key=osp.join("00001", "0266")),
            dict(lq_path=lq_paths_2, gt_path=gt_paths_2, key=osp.join("00002", "0268")),
        ]
...
def test_vid4_dataset():
    ...
    with patch("builtins.open", mocked_open_function):
        ...
        results = [
            {"eval_result": {"PSNR": 21, "SSIM": 0.75}},
            {"eval_result": {"PSNR": 22, "SSIM": 0.8}},
            {"eval_result": {"PSNR": 24, "SSIM": 0.9}},
        ]
...
def test_sr_reds_multiple_gt_dataset():
    ...
    assert reds_dataset.data_infos[0] == dict(
        lq_path=str(root_path), gt_path=str(root_path), key="001", sequence_length=100, num_input_frames=20
    )  # 000 is been removed
    ...
    assert reds_dataset.data_infos[0] == dict(
        lq_path=str(root_path), gt_path=str(root_path), key="240", sequence_length=100, num_input_frames=5
    )
    ...
    assert reds_dataset.data_infos[1] == dict(
        lq_path=str(root_path), gt_path=str(root_path), key="011", sequence_length=100, num_input_frames=5
    )
...
def test_sr_test_multiple_gt_dataset():
    ...
    test_dataset = SRTestMultipleGTDataset(
        lq_folder=root_path, gt_folder=root_path, pipeline=[], scale=4, test_mode=True
    )

    assert test_dataset.data_infos == [
        dict(lq_path=str(root_path), gt_path=str(root_path), key="sequence_1", sequence_length=2),
        dict(lq_path=str(root_path), gt_path=str(root_path), key="sequence_2", sequence_length=1),
    ]
...
def test_sr_folder_multiple_gt_dataset():
    ...
    assert test_dataset.data_infos == [
        dict(lq_path=str(root_path), gt_path=str(root_path), key="sequence_1", num_input_frames=2, sequence_length=2),
        dict(lq_path=str(root_path), gt_path=str(root_path), key="sequence_2", num_input_frames=1, sequence_length=1),
    ]

    # test with num_input_frames
    test_dataset = SRFolderMultipleGTDataset(
        lq_folder=root_path, gt_folder=root_path, pipeline=[], scale=4, num_input_frames=1, test_mode=True
    )
    assert test_dataset.data_infos == [
        dict(lq_path=str(root_path), gt_path=str(root_path), key="sequence_1", num_input_frames=1, sequence_length=2),
        dict(lq_path=str(root_path), gt_path=str(root_path), key="sequence_2", num_input_frames=1, sequence_length=1),
    ]
    ...
    with patch("builtins.open", mocked_open_function):
        # test without num_input_frames
        test_dataset = SRFolderMultipleGTDataset(
            lq_folder=root_path, gt_folder=root_path, pipeline=[], scale=4, ann_file="fake_ann_file", test_mode=True
        )
        assert test_dataset.data_infos == [
            dict(
                lq_path=str(root_path), gt_path=str(root_path), key="sequence_1", num_input_frames=2, sequence_length=2
            ),
        ]
        ...
        assert test_dataset.data_infos == [
            dict(
                lq_path=str(root_path), gt_path=str(root_path), key="sequence_1", num_input_frames=1, sequence_length=2
            ),
        ]
...
def test_sr_folder_video_dataset():
    ...
    test_dataset = SRFolderVideoDataset(
        lq_folder=root_path, gt_folder=root_path, num_input_frames=5, pipeline=[], scale=4, test_mode=True
    )
    ...
    with pytest.raises(AssertionError):
        SRFolderVideoDataset(
            lq_folder=root_path, gt_folder=root_path, num_input_frames=6, pipeline=[], scale=4, test_mode=True
        )
