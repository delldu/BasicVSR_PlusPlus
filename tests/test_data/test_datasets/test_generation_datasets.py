...
from mmedit.datasets import BaseGenerationDataset, GenerationPairedDataset, GenerationUnpairedDataset
...
class TestGenerationDatasets:
    ...
    def test_generation_paired_dataset(self):
        ...
        pipeline = [
            dict(type="LoadPairedImageFromFile", io_backend="disk", key="pair", flag="color"),
            dict(type="Resize", keys=["img_a", "img_b"], scale=(286, 286), interpolation="bicubic"),
            ...
            dict(type="Normalize", keys=["img_a", "img_b"], to_rgb=True, **img_norm_cfg),
            dict(type="ImageToTensor", keys=["img_a", "img_b"]),
            dict(type="Collect", keys=["img_a", "img_b"], meta_keys=["img_a_path", "img_b_path"]),
        ]
    ...
    def test_generation_unpaired_dataset(self):
        ...
        img_norm_cfg = dict(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
        pipeline = [
            dict(type="LoadImageFromFile", io_backend="disk", key="img_a", flag="color"),
            dict(type="LoadImageFromFile", io_backend="disk", key="img_b", flag="color"),
            dict(type="Resize", keys=["img_a", "img_b"], scale=(286, 286), interpolation="bicubic"),
            dict(type="Crop", keys=["img_a", "img_b"], crop_size=(256, 256), random_crop=True),
