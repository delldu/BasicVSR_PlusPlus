...
def test_base_mattor():
    backbone = dict(
        type="SimpleEncoderDecoder", encoder=dict(type="VGG16", in_channels=4), decoder=dict(type="PlainDecoder")
    )
    refiner = dict(type="PlainRefiner")
    train_cfg = mmcv.ConfigDict(train_backbone=True, train_refiner=True)
    test_cfg = mmcv.ConfigDict(refine=True, metrics=["SAD", "MSE", "GRAD", "CONN"])

    with pytest.raises(KeyError):
        # metrics should be specified in test_cfg
        BaseMattor(backbone, refiner, train_cfg.copy(), test_cfg=mmcv.ConfigDict(refine=True))

    with pytest.raises(KeyError):
        # supported metric should be one of {'SAD', 'MSE'}
        BaseMattor(
            backbone, refiner, train_cfg.copy(), test_cfg=mmcv.ConfigDict(refine=True, metrics=["UnsupportedMetric"])
        )

    with pytest.raises(TypeError):
        # metrics must be None or a list of str
        BaseMattor(backbone, refiner, train_cfg.copy(), test_cfg=mmcv.ConfigDict(refine=True, metrics="SAD"))

    # build mattor without refiner
    mattor = BaseMattor(backbone, refiner=None, train_cfg=None, test_cfg=test_cfg.copy())
    assert not mattor.with_refiner

    # only train the refiner, this will freeze the backbone
    mattor = BaseMattor(
        backbone, refiner, train_cfg=mmcv.ConfigDict(train_backbone=False, train_refiner=True), test_cfg=test_cfg.copy()
    )
    ...
    mattor = BaseMattor(
        backbone, refiner, train_cfg=mmcv.ConfigDict(train_backbone=True, train_refiner=False), test_cfg=test_cfg.copy()
    )
...
def test_dim():
    ...
    assert_dict_keys_equal(output_train["losses"], ["loss_alpha", "loss_comp", "loss_refine"])
    ...
    with torch.no_grad():
        ...
        assert_dict_keys_equal(output_test["eval_result"], ["SAD", "MSE", "GRAD", "CONN"])
...
def test_indexnet():
    ...
    with torch.no_grad():
        ...
        if torch.cuda.is_available():
            indexnet = build_model(model_cfg, train_cfg=None, test_cfg=test_cfg).cuda()
            indexnet.eval()
            input_test = _demo_input_test((64, 64), cuda=True)
            output_test = indexnet(**input_test, test_mode=True)
            assert isinstance(output_test["pred_alpha"], np.ndarray)
    ...
    indexnet = build_model(model_cfg, train_cfg=mmcv.ConfigDict(train_backbone=True), test_cfg=test_cfg)
    ...
    if torch.cuda.is_available():
        ...
        indexnet = build_model(model_cfg, train_cfg=mmcv.ConfigDict(train_backbone=True), test_cfg=test_cfg).cuda()
...
def test_gca():
    ...
    outputs = model(inputs["merged"], inputs["trimap"], inputs["meta"], inputs["alpha"])
    ...
    with torch.no_grad():
        ...
        assert_dict_keys_equal(outputs["eval_result"], ["SAD", "MSE", "GRAD", "CONN"])
...
def _demo_input_train(img_shape, batch_size=1, cuda=False):
    ...
    trimap = torch.from_numpy(np.random.randint(255, size=gray_shape).astype(np.float32))
    meta = [{}] * batch_size
    alpha = torch.from_numpy(np.random.random(gray_shape).astype(np.float32))
    ori_merged = torch.from_numpy(np.random.random(color_shape).astype(np.float32))
    ...
    return dict(merged=merged, trimap=trimap, meta=meta, alpha=alpha, ori_merged=ori_merged, fg=fg, bg=bg)
...
def _demo_input_test(img_shape, batch_size=1, cuda=False, test_trans="resize"):
    ...
    if cuda:
        merged = merged.cuda()
        trimap = trimap.cuda()
    meta = [dict(ori_alpha=ori_alpha, ori_trimap=ori_trimap, merged_ori_shape=img_shape)] * batch_size
