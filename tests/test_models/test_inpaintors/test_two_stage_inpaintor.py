...
def test_two_stage_inpaintor():
    model = dict(
        ...
        encdec=dict(
            ...
            stage1=dict(
                ...
                encoder=dict(type="DeepFillEncoder", conv_type="gated_conv", channel_factor=0.75),
                decoder=dict(type="DeepFillDecoder", conv_type="gated_conv", in_channels=96, channel_factor=0.75),
                dilation_neck=dict(
                    type="GLDilationNeck", in_channels=96, conv_type="gated_conv", act_cfg=dict(type="ELU")
                ),
            ),
            stage2=dict(
                type="DeepFillRefiner",
                encoder_attention=dict(
                    type="DeepFillEncoder", encoder_type="stage2_attention", conv_type="gated_conv", channel_factor=0.75
                ),
                encoder_conv=dict(
                    type="DeepFillEncoder", encoder_type="stage2_conv", conv_type="gated_conv", channel_factor=0.75
                ),
                dilation_neck=dict(
                    type="GLDilationNeck", in_channels=96, conv_type="gated_conv", act_cfg=dict(type="ELU")
                ),
                contextual_attention=dict(type="ContextualAttentionNeck", in_channels=96, conv_type="gated_conv"),
                decoder=dict(type="DeepFillDecoder", in_channels=192, conv_type="gated_conv"),
            ),
        ),
        disc=dict(
    ...
    tsinpaintor = TwoStageInpaintor(**model, train_cfg=train_cfg, test_cfg=test_cfg)

    # check architecture
    assert tsinpaintor.stage1_loss_type == ("loss_l1_hole", "loss_l1_valid")
    assert tsinpaintor.stage2_loss_type == ("loss_l1_hole", "loss_l1_valid", "loss_gan")
    ...
    if torch.cuda.is_available():
        ...
        mask[..., 50:180, 60:170] = 1.0
        masked_img = gt_img * (1.0 - mask)

        data_batch = dict(gt_img=gt_img, mask=mask, masked_img=masked_img)

        # prepare model and optimizer
        tsinpaintor.cuda()
        optimizers_config = dict(generator=dict(type="Adam", lr=0.0001), disc=dict(type="Adam", lr=0.0001))
        ...
        outputs = tsinpaintor.forward_test(masked_img[0:1], mask[0:1], gt_img=gt_img[0:1, ...])
        ...
        for key in ["stage1_fake_res", "stage2_fake_res", "fake_res", "fake_img"]:
        ...
        with pytest.raises(NotImplementedError):
            ...
            tsinpaintor = TwoStageInpaintor(**model_, train_cfg=train_cfg, test_cfg=test_cfg).cuda()
            outputs = tsinpaintor.train_step(data_batch, optims)
        ...
        model_ = dict(
            ...
            encdec=dict(
                ...
                stage1=dict(
                    type="GLEncoderDecoder",
                    encoder=dict(type="DeepFillEncoder", in_channels=4, conv_type="gated_conv", channel_factor=0.75),
                    decoder=dict(type="DeepFillDecoder", conv_type="gated_conv", in_channels=96, channel_factor=0.75),
                    dilation_neck=dict(
                        type="GLDilationNeck", in_channels=96, conv_type="gated_conv", act_cfg=dict(type="ELU")
                    ),
