...
def test_gl_inpaintor():
    ...
    if torch.cuda.is_available():
        ...
        mask[..., 100:210, 100:210] = 1.0
        masked_img = gt_img * (1.0 - mask)
        mask_bbox = torch.tensor([[100, 100, 110, 110]])
        gl.cuda()
        data_batch = dict(
            gt_img=gt_img.cuda(), mask=mask.cuda(), masked_img=masked_img.cuda(), mask_bbox=mask_bbox.cuda()
        )
        ...
        gl_dirty = build_model(cfg.model_dirty, train_cfg=cfg.train_cfg, test_cfg=cfg.test_cfg)
        gl_dirty.cuda()
