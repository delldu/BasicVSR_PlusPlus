...
def test_two_stage_inpaintor():
    ...
    tsinpaintor = DeepFillv1Inpaintor(**model, train_cfg=train_cfg, test_cfg=test_cfg)
    ...
    if torch.cuda.is_available():
        ...
        bbox_tensor = torch.tensor([[50, 60, 110, 110], [50, 60, 110, 110]]).cuda()

        data_batch = dict(gt_img=gt_img, mask=mask, masked_img=masked_img, mask_bbox=bbox_tensor)
        ...
        with pytest.raises(NotImplementedError):
            ...
            tsinpaintor = DeepFillv1Inpaintor(**model_, train_cfg=train_cfg, test_cfg=test_cfg).cuda()
            outputs = tsinpaintor.train_step(data_batch, optims)
        ...
        model_ = dict(
            ...
            encdec=dict(
                ...
                stage1=dict(
                    ...
                    dilation_neck=dict(type="GLDilationNeck", in_channels=128, act_cfg=dict(type="ELU")),
                ),
                stage2=dict(
                    type="DeepFillRefiner",
                    encoder_attention=dict(type="DeepFillEncoder", encoder_type="stage2_attention", in_channels=4),
                    encoder_conv=dict(type="DeepFillEncoder", encoder_type="stage2_conv", in_channels=4),
                ),
            ),
            disc=dict(
                type="DeepFillv1Discriminators",
