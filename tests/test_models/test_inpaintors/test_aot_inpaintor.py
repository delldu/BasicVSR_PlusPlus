...
def test_aot_inpaintor():
    ...
    inpaintor = build_model(cfg.model, train_cfg=cfg.train_cfg, test_cfg=cfg.test_cfg)
    ...
    mask[..., 20:100, 100:120] = 1.0
    masked_img = gt_img * (1.0 - mask) + mask
    inpaintor
    ...
    outputs = inpaintor.forward_test(masked_img[0:1], mask[0:1], gt_img=gt_img[0:1, ...])
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with pytest.raises(AssertionError):
            outputs = inpaintor.forward_test(
                masked_img[0:1], mask[0:1], save_image=True, save_path=tmpdir, iteration=4396, gt_img=gt_img[0:1, ...]
            )
        with pytest.raises(AssertionError):
            outputs = inpaintor.forward_test(
                masked_img[0:1],
    ...
    inpaintor = build_model(cfg_.model, train_cfg=cfg_.train_cfg, test_cfg=cfg_.test_cfg)
