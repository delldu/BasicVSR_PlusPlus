...
from mmedit.models.common import (
    ASPP,
...
def test_mask_conv_module():
    ...
    mask_in[..., 20:130, 120:150] = 0.0
    output, mask_update = conv(x, mask_in)
    assert output.shape == (1, 8, 255, 255)
    assert mask_update.shape == (1, 8, 255, 255)

    # add test for ['norm', 'conv', 'act']
    conv = MaskConvModule(3, 8, 2, order=("norm", "conv", "act"), conv_cfg=conv_cfg)
    x = torch.rand(1, 3, 256, 256)
    output = conv(x, mask_in, return_mask=False)
    assert output.shape == (1, 8, 255, 255)

    conv = MaskConvModule(3, 8, 3, padding=1, conv_cfg=conv_cfg, with_spectral_norm=True)
    assert hasattr(conv.conv, "weight_orig")
    output = conv(x, return_mask=False)
    assert output.shape == (1, 8, 256, 256)

    conv = MaskConvModule(3, 8, 3, padding=1, norm_cfg=dict(type="BN"), padding_mode="reflect", conv_cfg=conv_cfg)
    assert isinstance(conv.padding_layer, nn.ReflectionPad2d)
    output = conv(x, mask_in, return_mask=False)
    assert output.shape == (1, 8, 256, 256)

    conv = MaskConvModule(3, 8, 3, padding=1, act_cfg=dict(type="LeakyReLU"), conv_cfg=conv_cfg)
    output = conv(x, mask_in, return_mask=False)
    assert output.shape == (1, 8, 256, 256)
...
def test_pconv2d():
    pconv2d = PartialConv2d(3, 2, kernel_size=1, stride=1, multi_channel=True, eps=1e-8)
    ...
    pconv2d = PartialConv2d(3, 2, kernel_size=1, stride=1, multi_channel=False, eps=1e-8)
    output = pconv2d(x, mask=None)
    assert output.shape == (1, 2, 6, 6)

    pconv2d = PartialConv2d(3, 2, kernel_size=1, stride=1, bias=False, multi_channel=True, eps=1e-8)
    ...
    pconv2d = PartialConv2d(3, 2, kernel_size=1, stride=1, bias=False, multi_channel=False, eps=1e-8)
    output = pconv2d(x, mask=None)
    assert output.shape == (1, 2, 6, 6)
    ...
    if torch.cuda.is_available():
        pconv2d = PartialConv2d(3, 2, kernel_size=1, stride=1, bias=False, multi_channel=False, eps=1e-8).cuda().half()
...
def test_gated_conv():
    ...
    conv = SimpleGatedConvModule(3, 10, 3, padding=1, feat_act_cfg=None, gate_act_cfg=None)
    assert not conv.with_gate_act
    out = conv(x)
    assert out.shape == (2, 10, 10, 10)

    with pytest.raises(AssertionError):
        conv = SimpleGatedConvModule(3, 1, 3, padding=1, order=("linear", "act", "norm"))
...
def test_linear_module():
    ...
    linear = LinearModule(10, 20, act_cfg=dict(type="LeakyReLU"), with_spectral_norm=True)
    y = linear(x)
    assert y.shape == (3, 20)
    assert isinstance(linear.activate, nn.LeakyReLU)

    linear = LinearModule(10, 20, bias=False, act_cfg=None, with_spectral_norm=True)
    y = linear(x)
    assert y.shape == (3, 20)
    assert not linear.with_bias

    linear = LinearModule(10, 20, bias=False, act_cfg=None, with_spectral_norm=True, order=("act", "linear"))
