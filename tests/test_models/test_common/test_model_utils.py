...
def test_gan_image_buffer():
    ...
    img_tensor_return = buffer.query(img_tensor)
    assert (torch.equal(img_tensor_return, img_tensor) and torch.equal(buffer.image_buffer[0], img_tensor_0)) or (
        torch.equal(img_tensor_return, img_tensor_00) and torch.equal(buffer.image_buffer[0], img_tensor_1)
    )
...
def test_extract_around_bbox():
    ...
    img_np = np.random.randn(100, 100, 3)
    bbox = np.asarray([10, 10, 10, 10])
    img_new, bbox_new = extract_around_bbox(img_np, bbox, (14, 14), channel_first=False)
    ...
    img_new, bbox_new = extract_around_bbox(img_tensor, bbox_tensor, target_size=[14, 14])
