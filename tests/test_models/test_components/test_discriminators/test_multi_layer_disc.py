...
def test_multi_layer_disc():
    ...
    multi_disc = MultiLayerDiscriminator(in_channels=3, max_channels=256, fc_in_channels=None)
    multi_disc.init_weights()
    disc_pred = multi_disc(input_g)
    assert disc_pred.shape == (1, 256, 8, 8)
    multi_disc = MultiLayerDiscriminator(in_channels=3, max_channels=256, fc_in_channels=100)
    ...
    multi_disc = MultiLayerDiscriminator(3, 236, fc_in_channels=None, out_act_cfg=None)
    ...
    for i in range(1, num_convs):
        ...
        assert isinstance(getattr(multi_disc, f"conv{i + 1}").activate, nn.LeakyReLU)
