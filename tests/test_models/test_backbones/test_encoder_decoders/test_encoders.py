...
from mmedit.models.backbones import (
    VGG16,
    DepthwiseIndexBlock,
    FBAResnetDilated,
    HolisticIndexBlock,
...
from mmedit.models.backbones.encoder_decoders.encoders.resnet import BasicBlock, Bottleneck
...
def test_indexnet_encoder():
    ...
    indexnet_encoder = IndexNetEncoder(
        4, out_stride=32, width_mult=1, index_mode="m2o", aspp=True, use_nonlinear=True, use_context=True
    )
    ...
    indexnet_encoder = IndexNetEncoder(
        4, out_stride=16, width_mult=2, index_mode="o2o", aspp=False, use_nonlinear=False, use_context=False
    )
    indexnet_encoder.init_weights()
    x = torch.rand(2, 4, 32, 32)
    outputs = indexnet_encoder(x)
...
def test_fba_encoder():
    ...
    with pytest.raises(AssertionError):
        ...
        FBAResnetDilated(
            50, in_channels=11, stem_channels=64, base_channels=64, strides=(1,), dilations=(1, 1), num_stages=3
        )
