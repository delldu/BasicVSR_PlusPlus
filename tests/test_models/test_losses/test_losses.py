...
def test_utils():
    ...
    target = (loss * weight).sum(dim=[1, 2, 3]) / weight.sum(dim=[1, 2, 3]).mean()
    ...
    reduced = mask_reduce_loss(loss, weight=weight_single_channel, reduction="mean")
    target = (loss * weight).sum(dim=[1, 2, 3]) / weight.sum(dim=[1, 2, 3]).mean()
    npt.assert_almost_equal(reduced.numpy(), target)
    ...
    target = (loss_b * weight_b).sum() / weight_b.sum() / 3.0
    npt.assert_almost_equal(reduced.numpy(), target)
...
def test_pixelwise_losses():
    ...
    charbonnier_loss = CharbonnierLoss(loss_weight=1.0, reduction="mean", eps=1e-12)
    loss = charbonnier_loss(pred, target)
    assert loss.shape == ()
    assert math.isclose(loss.item(), 0.25, rel_tol=1e-5)

    charbonnier_loss = CharbonnierLoss(loss_weight=0.5, reduction="none", eps=1e-6)
    loss = charbonnier_loss(pred, target, weight)
    assert loss.shape == (1, 1, 64, 64)
    npt.assert_almost_equal(loss.numpy(), torch.ones(1, 1, 64, 64) * weight * 0.5, decimal=6)

    charbonnier_loss = CharbonnierLoss(loss_weight=0.5, reduction="sum", eps=1e-12)
    loss = charbonnier_loss(pred, target)
    assert loss.shape == ()
...
def test_composition_losses():
    ...
    cb_comp_loss = CharbonnierCompLoss(loss_weight=1.0, reduction="mean", eps=1e-12)
    loss = cb_comp_loss(pred_alpha, fg, bg, ori_merged)
    assert loss.shape == ()
    assert loss.item() == 2.5

    cb_comp_loss = CharbonnierCompLoss(loss_weight=0.5, reduction="none", eps=1e-6)
    loss = cb_comp_loss(pred_alpha, fg, bg, ori_merged, weight)
    assert loss.shape == (1, 3, 64, 64)
    npt.assert_almost_equal(loss.numpy(), torch.ones(1, 3, 64, 64) * weight * 0.5, decimal=6)

    cb_comp_loss = CharbonnierCompLoss(loss_weight=0.5, reduction="sum", eps=1e-6)
    loss = cb_comp_loss(pred_alpha, fg, bg, ori_merged, weight)
    assert loss.shape == ()
...
def test_perceptual_loss(init_weights):
    if torch.cuda.is_available():
        loss_percep = PerceptualLoss(layer_weights={"0": 1.0}).cuda()
        ...
        loss_percep = PerceptualLoss(layer_weights={"0": 1.0}, perceptual_weight=0.0).cuda()
        x = torch.randn(1, 3, 16, 16).cuda()
        gt = torch.randn(1, 3, 16, 16).cuda()
        percep, style = loss_percep(x, gt)
        assert percep is None and style > 0

        loss_percep = PerceptualLoss(layer_weights={"0": 1.0}, style_weight=0.0, criterion="mse").cuda()
        x = torch.randn(1, 3, 16, 16).cuda()
        gt = torch.randn(1, 3, 16, 16).cuda()
        percep, style = loss_percep(x, gt)
        assert style is None and percep > 0

        loss_percep = PerceptualLoss(layer_weights={"0": 1.0}, layer_weights_style={"1": 1.0}).cuda()
        x = torch.randn(1, 3, 16, 16).cuda()
        gt = torch.randn(1, 3, 16, 16).cuda()
        percep, style = loss_percep(x, gt)
    ...
    with pytest.raises(AssertionError):
        loss_percep = PerceptualLoss(layer_weights={"0": 1.0}, vgg_type="igccc")
    # test whether criterion is valid
    with pytest.raises(NotImplementedError):
        loss_percep = PerceptualLoss(layer_weights={"0": 1.0}, criterion="igccc")

    layer_name_list = ["2", "10", "30"]
    vgg_model = PerceptualVGG(layer_name_list, use_input_norm=False, vgg_type="vgg16", pretrained="torchvision://vgg16")
    ...
    with pytest.raises(AssertionError):
        ...
        vgg_model = PerceptualVGG(
            layer_name_list, use_input_norm=False, vgg_type="vgg16", pretrained="torchvision://vgg16"
        )
...
def test_t_perceptual_loss():
    ...
    maps = [torch.rand((2, 8, 8, 8), requires_grad=True), torch.rand((2, 4, 16, 16), requires_grad=True)]
    ...
    loss_t_percep = TransferalPerceptualLoss(use_attention=False, criterion="l1")
    t_percep = loss_t_percep(maps, soft, textures)
    assert t_percep.item() > 0

    if torch.cuda.is_available():
        maps = [torch.rand((2, 8, 8, 8)).cuda(), torch.rand((2, 4, 16, 16)).cuda()]
        textures = [torch.rand((2, 8, 8, 8)).cuda(), torch.rand((2, 4, 16, 16)).cuda()]
        ...
        loss_t_percep = TransferalPerceptualLoss(use_attention=False, criterion="l1").cuda()
        t_percep = loss_t_percep(maps, soft, textures)
        assert t_percep.item() > 0
...
def test_gan_losses():
    """Test gan losses."""
    with pytest.raises(NotImplementedError):
        GANLoss("xixihaha", loss_weight=1.0, real_label_val=1.0, fake_label_val=0.0)
    ...
    # lsgan
    gan_loss = GANLoss("lsgan", loss_weight=2.0, real_label_val=1.0, fake_label_val=0.0)
    ...
    # wgan
    gan_loss = GANLoss("wgan", loss_weight=2.0, real_label_val=1.0, fake_label_val=0.0)
    loss = gan_loss(input_2, True, is_disc=False)
    npt.assert_almost_equal(loss.item(), -4.0)
    loss = gan_loss(input_2, False, is_disc=False)
    ...
    gan_loss = GANLoss("smgan", loss_weight=2.0, real_label_val=1.0, fake_label_val=0.0)
