...
def test_cyclegan():
    ...
    model_cfg = dict(
        ...
        gan_loss=dict(type="GANLoss", gan_type="lsgan", real_label_val=1.0, fake_label_val=0, loss_weight=1.0),
    ...
    # build synthesizer
    synthesizer = build_model(model_cfg, train_cfg=train_cfg, test_cfg=test_cfg)
    ...
    optimizer = {
        "generators": obj_from_dict(
            optim_cfg, torch.optim, dict(params=getattr(synthesizer, "generators").parameters())
        ),
        "discriminators": obj_from_dict(
            optim_cfg, torch.optim, dict(params=getattr(synthesizer, "discriminators").parameters())
        ),
    }
    ...
    if torch.cuda.is_available():
        ...
        with torch.no_grad():
            outputs = synthesizer(
                data_batch_cuda["img_a"], data_batch_cuda["img_b"], data_batch_cuda["meta"], test_mode=True
            )
        ...
        # test forward_train
        outputs = synthesizer(
            data_batch_cuda["img_a"], data_batch_cuda["img_b"], data_batch_cuda["meta"], test_mode=False
        )
        ...
        for v in [
            ...
            assert isinstance(outputs["log_vars"][v], float)
        assert outputs["num_samples"] == 1
        assert torch.equal(outputs["results"]["real_a"], data_batch_cuda["img_a"].cpu())
        assert torch.equal(outputs["results"]["real_b"], data_batch_cuda["img_b"].cpu())
        assert torch.is_tensor(outputs["results"]["fake_b"])
        assert torch.is_tensor(outputs["results"]["fake_a"])
        assert outputs["results"]["fake_b"].size() == (1, 3, 64, 64)
        assert outputs["results"]["fake_a"].size() == (1, 3, 64, 64)
    ...
    for i in range(2):
        ...
        assert isinstance(outputs["log_vars"], dict)
        assert isinstance(outputs["results"], dict)
        for v in ["loss_id_a", "loss_id_b", "loss_gan_g_a", "loss_gan_g_b", "loss_cycle_a", "loss_cycle_b"]:
    ...
    for i in range(2, 6):
        ...
        if i % 2 == 1:
            log_None_list = ["loss_id_a", "loss_id_b", "loss_gan_g_a", "loss_gan_g_b", "loss_cycle_a", "loss_cycle_b"]
    ...
    log_check_list = ["loss_gan_d_a", "loss_gan_d_b", "loss_gan_g_a", "loss_gan_g_b", "loss_cycle_a", "loss_cycle_b"]
    for v in log_check_list:
        assert isinstance(outputs["log_vars"][v], float)
    assert outputs["num_samples"] == 1
    assert torch.equal(outputs["results"]["real_a"], data_batch["img_a"])
    ...
    with patch.object(mmcv, "imwrite", return_value=True):
        ...
        with pytest.raises(AssertionError):
            with torch.no_grad():
                _ = synthesizer(inputs, targets, [img_meta], test_mode=True, save_image=True)
        # iteration is None
        with torch.no_grad():
            outputs = synthesizer(inputs, targets, [img_meta], test_mode=True, save_image=True, save_path="save_path")
        assert torch.equal(outputs["real_a"], data_batch["img_a"])
        assert torch.equal(outputs["real_b"], data_batch["img_b"])
        assert torch.is_tensor(outputs["fake_b"])
        assert torch.is_tensor(outputs["fake_a"])
        ...
        with torch.no_grad():
            outputs = synthesizer(
                inputs, targets, [img_meta], test_mode=True, save_image=True, save_path="save_path", iteration=1000
            )
        assert torch.equal(outputs["real_a"], data_batch["img_a"])
        assert torch.equal(outputs["real_b"], data_batch["img_b"])
        assert torch.is_tensor(outputs["fake_b"])
