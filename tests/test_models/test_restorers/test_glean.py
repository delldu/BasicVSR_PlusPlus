...
def test_glean():
    ...
    model_cfg = dict(
        type="GLEAN",
        generator=dict(type="GLEANStyleGANv2", in_size=16, out_size=64, style_channels=512),
        discriminator=dict(type="StyleGAN2Discriminator", in_size=64),
        pixel_loss=dict(type="L1Loss", loss_weight=1.0, reduction="mean"),
        gan_loss=dict(type="GANLoss", gan_type="vanilla", real_label_val=1.0, fake_label_val=0, loss_weight=5e-3),
    )
    ...
    with pytest.raises(ValueError):  # iteration is not None or number
        with torch.no_grad():
            restorer(**data_batch, test_mode=True, save_image=True, meta=meta, iteration="1")
