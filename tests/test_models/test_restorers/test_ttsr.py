...
from mmedit.models.backbones.sr_backbones.ttsr_net import CSFI2, CSFI3, SFE, MergeFeatures
...
def test_ttsr_net():
    ...
    ttsr_cfg = dict(type="TTSRNet", in_channels=3, out_channels=3, mid_channels=16, texture_channels=16)
...
def test_ttsr():
    model_cfg = dict(
        type="TTSR",
        generator=dict(type="TTSRNet", in_channels=3, out_channels=3, mid_channels=64, num_blocks=(16, 16, 8, 4)),
        ...
        transferal_perceptual_loss=dict(
            type="TransferalPerceptualLoss", loss_weight=1e-2, use_attention=False, criterion="mse"
        ),
        gan_loss=dict(type="GANLoss", gan_type="vanilla", loss_weight=1e-3, real_label_val=1.0, fake_label_val=0),
    )
    ...
    data_batch = {"lq": inputs, "gt": targets, "ref": ref, "lq_up": ref, "ref_downup": ref}

    # prepare optimizer
    optim_cfg_g = dict(type="Adam", lr=1e-4, betas=(0.9, 0.999))
    optim_cfg_d = dict(type="Adam", lr=1e-4, betas=(0.9, 0.999))
    optimizer = dict(
        generator=obj_from_dict(optim_cfg_g, torch.optim, dict(params=restorer.parameters())),
        discriminator=obj_from_dict(optim_cfg_d, torch.optim, dict(params=restorer.parameters())),
    )
