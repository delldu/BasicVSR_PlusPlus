...
def test_dic_model():
    ...
    model_cfg_pre = dict(
        type="DIC",
        generator=dict(type="DICNet", in_channels=3, out_channels=3, mid_channels=48),
    ...
    model_cfg = dict(
        ...
        pixel_loss=dict(type="L1Loss", loss_weight=1.0, reduction="mean"),
        align_loss=dict(type="MSELoss", loss_weight=0.1, reduction="mean"),
        feature_loss=dict(type="LightCNNFeatureLoss", pretrained=pretrained, loss_weight=0.1, criterion="l1"),
        gan_loss=dict(type="GANLoss", gan_type="vanilla", loss_weight=0.005, real_label_val=1.0, fake_label_val=0),
    )
    ...
    generator = obj_from_dict(optim_cfg, torch.optim, dict(params=restorer.parameters()))
    discriminator = obj_from_dict(optim_cfg, torch.optim, dict(params=restorer.parameters()))
    ...
    if torch.cuda.is_available():
        ...
        data_batch = {"lq": inputs.cuda(), "gt": targets.cuda(), "heatmap": heatmap.cuda()}
