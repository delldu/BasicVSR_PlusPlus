...
def test_cain():
    model_cfg = dict(
        type="CAIN", generator=dict(type="CAINNet"), pixel_loss=dict(type="L1Loss", loss_weight=1.0, reduction="mean")
    )
    ...
    if torch.cuda.is_available():
        ...
        optimizer["generator"] = obj_from_dict(optim_cfg, torch.optim, dict(params=restorer.parameters()))
        data_batch = {"inputs": inputs.cuda(), "target": target.cuda(), "meta": [{"key": "001"}]}
    ...
    data_batch = {"inputs": inputs, "target": target, "meta": [{"key": "fake_path/fake_name"}]}
