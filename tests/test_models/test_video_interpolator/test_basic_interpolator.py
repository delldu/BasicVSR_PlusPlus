...
def test_basic_interpolator():
    ...
    optimizer = {"generator": obj_from_dict(optim_cfg, torch.optim, dict(params=restorer.parameters()))}
    ...
    assert outputs["output"].max() <= 1.0
    assert outputs["output"].min() >= 0.0
    ...
    if torch.cuda.is_available():
        ...
        assert isinstance(outputs, dict)
        assert isinstance(outputs["losses"], dict)
        assert isinstance(outputs["losses"]["loss_pix"], torch.cuda.FloatTensor)
        assert outputs["num_samples"] == 1
        assert torch.equal(outputs["results"]["inputs"], data_batch["inputs"].cpu())
        assert torch.equal(outputs["results"]["target"], data_batch["target"].cpu())
        assert torch.is_tensor(outputs["results"]["output"])
        assert outputs["results"]["output"].size() == (1, 3, 20, 20)
    ...
    data_batch = {
        "inputs": inputs,
        "target": target,
        "meta": [{"key": "000001/0000", "target_path": "fake_path/fake_name.png"}],
    }
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        outputs = restorer(
            inputs=inputs,
            target=target,
            meta=[{"key": "000001/0000", "inputs_path": ["fake_path/fake_name.png", "fake_path/fake_name.png"]}],
            test_mode=True,
            save_image=True,
            save_path=tmpdir,
            iteration=100,
