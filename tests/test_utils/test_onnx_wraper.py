...
def test_restorer_wrapper():
    ...
    cfg = dict(
        model=dict(
            type="BasicRestorer",
            generator=dict(type="SRCNN", channels=(3, 4, 2, 3), kernel_sizes=(9, 1, 5), upscale_factor=scale),
    ...
    pytorch_model = build_model(cfg.model, train_cfg=cfg.train_cfg, test_cfg=cfg.test_cfg)
...
def test_mattor_wrapper():
    ...
    data_batch["meta"] = [dict(ori_alpha=ori_alpha, ori_trimap=ori_trimap, merged_ori_shape=img_shape)]
