...
def test_pix2pix():
    ...
    model_cfg = dict(
        ...
        gan_loss=dict(type="GANLoss", gan_type="vanilla", real_label_val=1.0, fake_label_val=0, loss_weight=1.0),
    ...
    optimizer = {
        "generator": obj_from_dict(optim_cfg, torch.optim, dict(params=getattr(synthesizer, "generator").parameters())),
        "discriminator": obj_from_dict(
            optim_cfg, torch.optim, dict(params=getattr(synthesizer, "discriminator").parameters())
        ),
    }
    ...
    for v in ["loss_gan_d_fake", "loss_gan_d_real", "loss_gan_g", "loss_pixel"]:
    ...
    if torch.cuda.is_available():
        ...
        optimizer = {
            "generator": obj_from_dict(
                optim_cfg, torch.optim, dict(params=getattr(synthesizer, "generator").parameters())
            ),
            "discriminator": obj_from_dict(
                optim_cfg, torch.optim, dict(params=getattr(synthesizer, "discriminator").parameters())
            ),
    ...
    for i in range(2, 6):
        ...
        assert isinstance(outputs["log_vars"], dict)
        assert isinstance(outputs["results"], dict)
        log_check_list = ["loss_gan_d_fake", "loss_gan_d_real", "loss_gan_g", "loss_pixel"]
        if i % 2 == 1:
            assert outputs["log_vars"].get("loss_gan_g") is None
            assert outputs["log_vars"].get("loss_pixel") is None
