...
train_pipeline = [
    dict(type="GenerateSegmentIndices", interval_list=[1], filename_tmpl="{:05d}.png"),
...
data = dict(
    workers_per_gpu=6,
    train_dataloader=dict(samples_per_gpu=1, drop_last=True, persistent_workers=False),  # 8 gpus
...
optimizers = dict(
    generator=dict(
        type="Adam", lr=1e-4, betas=(0.9, 0.99), paramwise_cfg=dict(custom_keys={"spynet": dict(lr_mult=0.25)})
    )
)

# learning policy
total_iters = 200000
lr_config = dict(policy="CosineRestart", by_epoch=False, periods=[200000], restart_weights=[1], min_lr=1e-7)
