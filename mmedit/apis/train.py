...
def _dist_train(model, dataset, cfg, validate=False, logger=None, timestamp=None, meta=None):
    ...
    runner = IterBasedRunner(model, optimizer=optimizer, work_dir=cfg.work_dir, logger=logger, meta=meta)
    ...
    if validate and cfg.get("evaluation", None) is not None:
        ...
        if "val_samples_per_gpu" in cfg.data or "val_workers_per_gpu" in cfg.data:
            warnings.warn(
                '"val_samples_per_gpu/val_workers_per_gpu" have '
        ...
        runner.register_hook(DistEvalIterHook(data_loader, save_path=save_path, **cfg.evaluation), priority="LOW")
...
def _non_dist_train(model, dataset, cfg, validate=False, logger=None, timestamp=None, meta=None):
    ...
    # step 1: give default values and override (if exist) from cfg.data
    loader_cfg = {
        **dict(seed=cfg.get("seed"), drop_last=False, dist=False, num_gpus=cfg.gpus),
        **(
            {}
            if torch.__version__ != "parrots"
            else dict(
    ...
    if validate and cfg.get("evaluation", None) is not None:
        ...
        data_loader = build_dataloader(dataset, **val_loader_cfg)
        save_path = osp.join(cfg.work_dir, "val_visuals")
        runner.register_hook(EvalIterHook(data_loader, save_path=save_path, **cfg.evaluation), priority="LOW")
