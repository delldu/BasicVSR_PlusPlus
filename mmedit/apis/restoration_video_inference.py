...
def pad_sequence(data, window_size):
    ...
    data = torch.cat(
        [data[:, 1 + padding : 1 + 2 * padding].flip(1), data, data[:, -1 - 2 * padding : -1 - padding].flip(1)], dim=1
    )

    return data


def restoration_video_inference(model, img_dir, window_size, start_idx, filename_tmpl, max_seq_len=None):
    ...
    if file_extension in VIDEO_EXTENSIONS:  # False
        ...
        data = dict(lq_path=lq_folder, gt_path="", key=key, sequence_length=sequence_length)
    ...
    with torch.no_grad():
        if window_size > 0:  # sliding window framework
            ...
            if max_seq_len is None:
                ...
                result = model(lq=data.to(device), test_mode=True)["output"].cpu()
            else:
                result = []
                for i in range(0, data.size(1), max_seq_len):
                    result.append(model(lq=data[:, i : i + max_seq_len].to(device), test_mode=True)["output"].cpu())
