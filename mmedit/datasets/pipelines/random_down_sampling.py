...
class RandomDownSampling:
    ...
    def __init__(self, scale_min=1.0, scale_max=4.0, patch_size=None, interpolation="bicubic", backend="pillow"):
    ...
    def __call__(self, results):
        ...
        if self.patch_size is None:
            ...
            img_down = resize_fn(img, (w_lr, h_lr), self.interpolation, self.backend)
            ...
            crop_lr = resize_fn(crop_hr, w_lr, self.interpolation, self.backend)
        results["gt"] = crop_hr
        results["lq"] = crop_lr
        results["scale"] = scale
...
def resize_fn(img, size, interpolation="bicubic", backend="pillow"):
    ...
    if isinstance(img, np.ndarray):
        return imresize(img, size, interpolation=interpolation, backend=backend)
    elif isinstance(img, torch.Tensor):
        image = imresize(img.numpy(), size, interpolation=interpolation, backend=backend)
        return torch.from_numpy(image)

    else:
        raise TypeError("img should got np.ndarray or torch.Tensor," f"but got {type(img)}")
