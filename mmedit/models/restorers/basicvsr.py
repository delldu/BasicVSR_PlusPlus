...
class BasicVSR(BasicRestorer):
    ...
    def __init__(self, generator, pixel_loss, ensemble=None, train_cfg=None, test_cfg=None, pretrained=None):
    ...
    def evaluate(self, output, gt):
        ...
        for metric in self.test_cfg.metrics:
            if output.ndim == 5:  # a sequence: (n, t, c, h, w)
                ...
                value = self.allowed_metrics[metric](output_img, gt_img, crop_border, convert_to=convert_to)
    ...
    def forward_test(self, lq, gt=None, meta=None, save_image=False, save_path=None, iteration=None):
        ...
        if save_image:
            if output.ndim == 4:  # an image, key = 000001/0000 (Vimeo-90K)
                ...
                for i in range(0, output.size(1)):
                    if isinstance(iteration, numbers.Number):
                        save_path_i = osp.join(save_path, folder_name, f"{i:08d}-{iteration + 1:06d}.png")
                    elif iteration is None:
