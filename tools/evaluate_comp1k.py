...
def evaluate(pred_root, gt_root, trimap_root, verbose, nproc):
    ...
    if verbose:
        ...
        for i, img in enumerate(images):
            ...
            print(
                f"{img} SAD: {sad_result:.6g} MSE: {mse_result:.6g} " f"GRAD: {grad_result:.6g} CONN: {conn_result:.6g}"
            )

    sad_mean, mse_mean, grad_mean, conn_mean = np.mean(results, axis=0)
    print(f"MEAN:  SAD: {sad_mean:.6g} MSE: {mse_mean:.6g} " f"GRAD: {grad_mean:.6g} CONN: {conn_mean:.6g}")
...
def parse_args():
    ...
    parser.add_argument(
        "--trimap-root", help="Path to trimap folder. If not specified, " "results are calculated on the full image."
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Whether print result for each predicted alpha matte"
    )
...
def main():
    ...
    evaluate(args.pred_root, args.gt_root, args.trimap_root, args.verbose, args.nproc)
