...
def pytorch2onnx(
    model, input, model_type, opset_version=11, show=False, output_file="tmp.onnx", verify=False, dynamic_export=False
):
    ...
    if verify:
        ...
        assert np.allclose(
            pytorch_result, onnx_result, rtol=1e-5, atol=1e-5
...
def parse_args():
    ...
    parser.add_argument("--trimap-path", default=None, help="path to input trimap file, used in mattor model")
