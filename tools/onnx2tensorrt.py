...
from mmcv.tensorrt import TRTWrapper, is_tensorrt_plugin_loaded, onnx2trt, save_trt_engine
...
def _prepare_input_img(model_type: str, img_path: str, config: dict, rescale_shape: Optional[Iterable] = None) -> dict:
...
def onnx2tensorrt(
    ...
    if verify:
        ...
        if max_shape[0] > 1:
            ...
            img_list = [torch.cat((ori_img, flip_img), 0) for ori_img, flip_img in zip(img_list, flip_img_list)]
        ...
        onnx_output = sess.run(["output"], {"input": img_list[0].detach().numpy()})[0][0]
...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert MMSegmentation models from ONNX to TensorRT")
    ...
    parser.add_argument(
        "--max-shape", type=int, nargs=4, default=[1, 3, 512, 512], help="Maximum shape of model input."
    )
    parser.add_argument("--min-shape", type=int, nargs=4, default=[1, 3, 32, 32], help="Minimum shape of model input.")
...
if __name__ == "__main__":
    ...
    input_config = {"min_shape": args.min_shape, "max_shape": args.max_shape, "input_path": args.img_path}
